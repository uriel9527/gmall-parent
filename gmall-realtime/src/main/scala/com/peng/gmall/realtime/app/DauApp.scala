...
object DauApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //map遍历的每个元素是ConsumerRecord[String, String]类型的 分别是kafka的k,v
    ...
    val filteredDstream: DStream[StartupLog] = startupLogDstream.transform { rdd =>
      ...
      val jedis: Jedis = new Jedis("node1", 6379) //driver 每个执行周期查询redis获得清单 通过广播变量发送到executor中
    ...
    realFilteredDStream.foreachRDD { rdd =>
      ...
      rdd.foreachPartition { startuplogItr =>
        val jedis: Jedis = new Jedis("node1", 6379) //executor
